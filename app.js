  ...
  return {
    ...
    getPossibleMoves() {
      ...
      while (queue.length > 0) {
        ...
        let moves = this.getPossibleMovesFromPosition(current[0], current[1]);
    ...
    getPossibleMovesFromPosition(x, y) {
      let possibleMoves = [];
      switch (this.type) {
      ...
      case "Pawn":
        ...
        if (y + direction >= 0 && y + direction <= 7) {
          if (tiles[x][y + direction] === "") {
            possibleMoves.push([x, y + direction]);
            // double step
            if (!this.moved && tiles[x][y + direction * 2] === "") {
              possibleMoves.push([x, y + direction * 2]);

              if (y === 0 && this.color === "Black") {
                possibleMoves.push([x, y, "Queen"]);
                possibleMoves.push([x, y, "Rook"]);
                possibleMoves.push([x, y, "Bishop"]);
                possibleMoves.push([x, y, "Knight"]);
        ...
        // capture
        if (x + 1 <= 7) {
          if (tiles[x + 1][y + direction] !== "" && tiles[x + 1][y + direction].color !== this.color) {
            possibleMoves.push([x + 1, y + direction]);
          }
          // en passant
          if (this.color === "White" && y === 4) {
            let pawn = tiles[x + 1][y];
            if (pawn !== "" && pawn.color === "Black" && pawn.type === "Pawn" && pawn.moved === true) {
              possibleMoves.push([x + 1, y + direction]);
            }
          } else if (this.color === "Black" && y === 3) {
            let pawn = tiles[x + 1][y];
            if (pawn !== "" && pawn.color === "White" && pawn.type === "Pawn" && pawn.moved === true) {
              possibleMoves.push([x + 1, y + direction]);
            }
          }
        }
      ...
      case "King":
        ...
        for (let x = 0; x < 8; ++x) for (let y = 0; y < 8; ++y) tiles[y][x] = y % 2 === x % 2 ? "#D2B48C": "#A0522D";
        ...
        addEventListener("mousedown", e = >{
          ...
          if (moving === null) {
            ...
            if (moving === undefined || (moving.color === "White") === turn) moving = null;
        ...
        addEventListener("mouseup", e = >{
          if (moving !== null) {
            ...
            for (let x = 0; x < 8; ++x) for (let y = 0; y < 8; ++y) highlights[y][x] = "#00000000";
            if (checkmate) {
              winScreen = true;
        ...
        function getSquares(piece) {
          ...
          case "Pawn":
            ...
            if (getPieceAt(piece.x, piece.y - 1 * n) === undefined) {
              ...
              if (getPieceAt(piece.x, piece.y - 2 * n) === undefined && piece.y === r) validMoves.push([piece.x, piece.y - 2 * n, "#0000ff55"]);
            }
            const left = getPieceAt(piece.x - 1, piece.y - 1 * n);
            const right = getPieceAt(piece.x + 1, piece.y - 1 * n);
            if (left !== undefined && left.color !== piece.color) validMoves.push([piece.x - 1, piece.y - 1 * n, "#ff000055"]);
            if (right !== undefined && right.color !== piece.color) validMoves.push([piece.x + 1, piece.y - 1 * n, "#ff000055"]);
            break;
          case "Knight":
            const moves = [[piece.x + 2, piece.y + 1], [piece.x - 2, piece.y + 1], [piece.x + 2, piece.y - 1], [piece.x - 2, piece.y - 1], [piece.x + 1, piece.y + 2], [piece.x - 1, piece.y + 2], [piece.x + 1, piece.y - 2], [piece.x - 1, piece.y - 2], ];
            for (const m of moves) {
              const p = getPieceAt(...m);
              if (p === undefined) validMoves.push(m.concat("#0000ff55"));
              else if (p.color !== piece.color) validMoves.push(m.concat("#ff000055"));
            }
            break;
          case "Bishop":
            for (let dx = -1; dx < 2; dx += 2) for (let dy = -1; dy < 2; dy += 2) {
              let x = piece.x + dx;
              let y = piece.y + dy;
          ...
          case "King":
            for (let dx = -1; dx < 2; ++dx) for (let dy = -1; dy < 2; ++dy) {
              ...
              else if (p.color !== piece.color) validMoves.push([x, y, "#ff000055"]);
            }
            if (!piece.moved) {
              let left = getPieceAt(0, piece.y);
              let right = getPieceAt(7, piece.y);
              if (left !== undefined && !left.moved && getPieceAt(1, piece.y) === undefined && getPieceAt(2, piece.y) === undefined && getPieceAt(3, piece.y) === undefined) validMoves.push([piece.x - 2, piece.y, "#0000ff55"]) if (right !== undefined && !right.moved && getPieceAt(6, piece.y) === undefined && getPieceAt(5, piece.y) === undefined) validMoves.push([piece.x + 2, piece.y, "#0000ff55"])
            }
            break;
          }
        ...
        function inCheck() {
          ...
          for (const piece of pieces) if (getSquares(piece).find(a = >a[0] === king.x && a[1] === king.y)) return true;
          return false;
        }
        ...
        function reset() {
          ...
          pieces.splice(0, 0, Piece(0, 0, "Rook", "Black"), Piece(1, 0, "Knight", "Black"), Piece(2, 0, "Bishop", "Black"), Piece(3, 0, "Queen", "Black"), Piece(4, 0, "King", "Black"), Piece(5, 0, "Bishop", "Black"), Piece(6, 0, "Knight", "Black"), Piece(7, 0, "Rook", "Black"), Piece(0, 1, "Pawn", "Black"), Piece(1, 1, "Pawn", "Black"), Piece(2, 1, "Pawn", "Black"), Piece(3, 1, "Pawn", "Black"), Piece(4, 1, "Pawn", "Black"), Piece(5, 1, "Pawn", "Black"), Piece(6, 1, "Pawn", "Black"), Piece(7, 1, "Pawn", "Black"), Piece(0, 6, "Pawn", "White"), Piece(1, 6, "Pawn", "White"), Piece(2, 6, "Pawn", "White"), Piece(3, 6, "Pawn", "White"), Piece(4, 6, "Pawn", "White"), Piece(5, 6, "Pawn", "White"), Piece(6, 6, "Pawn", "White"), Piece(7, 6, "Pawn", "White"), Piece(0, 7, "Rook", "White"), Piece(1, 7, "Knight", "White"), Piece(2, 7, "Bishop", "White"), Piece(3, 7, "Queen", "White"), Piece(4, 7, "King", "White"), Piece(5, 7, "Bishop", "White"), Piece(6, 7, "Knight", "White"), Piece(7, 7, "Rook", "White"));
        }
      }
    }
  }
